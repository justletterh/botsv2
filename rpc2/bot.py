...
async def update_loop():
    while True:
        ...
        percnt=mem.percent
        si=level(percnt)
        name=f"{pmem(mem.total)} physical {pmem(swap.total)} swap"
        details=f"RAM Percent: {clean(percnt)}%"
        limg="764294359675043900"
        ltxt="RAM Metrics"
        simg=si.id
        stxt=si.txt
        pmin=pmem(mem.total-mem.available,str=False)
        pmax=pmem(mem.total,str=False)
        atype=discord.ActivityType.playing
        status="dnd"
        state=f"Swap Usage: {pmem(swap.total-swap.free)} of {pmem(swap.total)} | Physical (GB): "
        stamp=tstamp()
