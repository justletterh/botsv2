...
async def update_loop():
    while True:
        ...
        percnt=disk.percent
        si=level(percnt)
        name=f"{pmem(disk.total)} Total Space {len(psutil.disk_partitions())} Partition"
        if len(psutil.disk_partitions())!=1:
            name=f"{name}s"
        details=f"Usage Percent: {clean(percnt)}%"
        limg="764294359675043900"
        ltxt="Disk Metrics"
        simg=si.id
        stxt=si.txt
        pmin=pmem(disk.used,str=False)
        pmax=pmem(disk.total,str=False)
        atype=discord.ActivityType.playing
        status="dnd"
        state=f"{pmem(counters.read_bytes)} Read and {pmem(counters.write_bytes)} Written | Used (GB): "
        stamp=tstamp()
